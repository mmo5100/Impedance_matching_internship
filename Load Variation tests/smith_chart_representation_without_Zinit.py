...
# Short description of the load under test, shown as a subtitle below the
# chart title. Leave as "" to omit the subtitle entirely.
    ...
    for entry in data:
        for key in ("Z_init", "Z_gen", "Z_vna"):
            if entry.get(key) is None:
                continue  # entry doesn't have this point (e.g. unknown phase)
...
def draw_value_strip(ax, data, db_min, db_max, z0=Z0, cmap_name="tab10"):
    ...
    ax.text(1.0, -1.15, "Better match", ha="right", va="top",
            fontsize=8, transform=ax.transAxes)
    ax.set_title("Actual point colors, placed by S11 (dB)",
                 fontsize=9, pad=6)
...
def plot_positions(ax, data, db_min, db_max, z0=Z0, cmap_name="tab10"):
    ...
    for i, entry in enumerate(data):
        ...
        # --- Small marker: initial position (skipped if Z_init unknown,
        # e.g. only S11 in dB was available, with no phase information) ---
        if entry.get("Z_init") is not None:
...
def main():
    ...
    ax.set_title("Convergence Accuracy at 38 MHz: Generator vs. VNA Verification", fontsize=13)
    if LOAD_LABEL:
